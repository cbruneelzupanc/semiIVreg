...
vignette: >
  ...
  %\VignetteIndexEntry{semiIVreg: A package for semi-IV regressions in R}
...
In particular, `semiivreg` estimates the marginal treatment effect (MTE) and marginal treatment response (MTR). 
...
```
In general, different effect of the covariates across alternatives (i.e., \(\beta^X_{0} \neq \beta^X_{1}\)). To be specified by including the covariates in the semi-IVs: `semiivreg(y~d|w0+x|w1+x, data)`.
...
Estimate the propensity score \(\hat{P}\) of treatment selection of equation (3).  
...
\[
\begin{aligned}
E[Y|W_0, W_1, X, \hat{P}] = &D \times ( \delta_{1} + W_1 \beta_1 + X \beta^X_{1} + \kappa_1(\hat{P}) ) + \\
&(1-D) \times ( \delta_{0} + W_0 \beta_0 + X \beta^X_{0} + \kappa_0(\hat{P})).
\end{aligned}
\]

where the control functions are sample estimates (using the estimate \(\hat{P}\)) of their theoretical counterparts:
...
\[
\begin{aligned}
\hat{k}_1(u) &= E[ U_1 | U_D=u] = \hat{\kappa}_1(u) + u \hat{\kappa}_1'(u) \\
\hat{k}_0(u) &= E[ U_0 | U_D=u] = \hat{\kappa}_0(u) - (1-u) \hat{\kappa}_0'(u)
\end{aligned}
\]
...
\[
\begin{aligned}
\hat{MTR}_d(u, w_d, x) &= E(Y_d | X=x, W_d=w_d, U_D=u) = \hat{\delta}_{d} + w_d \hat{\beta}_d + x \hat{\beta}^X_{d} + \hat{k}_d(u)
\end{aligned}
\]
...
\[
\begin{aligned}
\hat{MTE}(u, x, w_0, w_1) = \hat{MTR}_1(u, w_1, x) - \hat{MTR}_0(u, w_0, x)
\end{aligned}
\]
...
\[
\hat{MTE}(x, w_0, w_1) = \hat{\delta}_{1} - \hat{\delta}_{0} + w_1 \hat{\beta}_1 - w_0 \hat{\beta}_0 + x (\hat{\beta}^X_{1} - \hat{\beta}^X_{0}).
\]
